...
namespace coordinates
{
  ...
  inline std::pair<glm::vec<L, int, Q>, glm::ivec2> split(glm::vec<L, int, Q> position)
  {
    ...
    local_position[0] = details::modulo(local_position[0], CHUNK_WIDTH);
    local_position[1] = details::modulo(local_position[1], CHUNK_WIDTH);

    glm::ivec2 chunk_index = glm::ivec2(
      (position[0] - local_position[0]) / CHUNK_WIDTH,
      (position[1] - local_position[1]) / CHUNK_WIDTH
    );
